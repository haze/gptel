...
(declare-function mcp-server-connections "mcp-hub")
(defvar mcp-hub--project-server-table)

(defun gptel-mcp-connect (&optional servers server-callback interactive project)
...
Call SERVER-CALLBACK after starting MCP servers."
  (interactive (list nil nil t (project-current)))
 ...
 or configure the mcp package"))
  ...
  (if (null (gethash project mcp-hub--project-server-table))
      (user-error "No MCP servers available!  Please configure `mcp-hub--project-server-table' via `mcp-hub-register-servers-for'")
    (let ((project-servers (gethash project mcp-hub--project-server-table)))
      (setq servers
            (if servers
                (mapcar (lambda (s) (assoc s project-servers)) servers)
              project-servers))
                    ...
                    (if interactive
                        (let ((picks
                          ...
                          (if (member "ALL" picks)
                            ...
                            (mapcar (lambda (s) (assoc s project-servers)) picks)))
                      unregistered-servers))
                   (server-active-p
                    (lambda (server)
                      (when-let* ((server (gethash (car server) (mcp-server-connections project))))
                   ...
                   (add-all-tools
                    (lambda (&optional server-names)
                      ...
                      (let ((tools (gptel-mcp--get-tools server-names))
                            (now-active (cl-remove-if-not server-active-p project-servers)))
                        (mapc (lambda (tool) (apply #'gptel-make-tool tool)) tools)
                        (gptel-mcp--activate-tools tools project)
...
(defun gptel-mcp-disconnect (&optional servers interactive project)
...
If INTERACTIVE is non-nil, query the user about which tools to remove."
  (interactive (list nil t (project-current)))
  (if-let* ((names-alist
             (cl-loop
        ...
        (if interactive
            (when-let* ((server-names
              ...
              (if (y-or-n-p
                  ...
                  (progn (mapc (lambda (name) (mcp-stop-server name project)) server-names)
    ...
    (if (cl-loop
         for v being the hash-values of (mcp-server-connections project)
         never v)
      ...
      (when (or (not interactive)
        ...
        (dolist (server (gethash project mcp-hub--project-server-table))
          (when (gethash (car server) (mcp-server-connections project))
            (mcp-stop-server (car server) project)))))))

(defun gptel-mcp--get-tools (&optional server-names project)
...
from all connected servers if it is nil."
  (unless server-names
    (setq server-names (hash-table-keys (mcp-server-connections project))))
  (let ((servers (mapcar (lambda (n) (gethash n (mcp-server-connections project)))
                         server-names)))
    (cl-mapcan
     (lambda (name server)
...
(defun gptel-mcp--activate-tools (&optional tools project)
  "Activate TOOLS or all MCP tools in current gptel session."
  (unless tools (setq tools (gptel-mcp--get-tools nil project)))
  (dolist (tool tools)
...
(with-eval-after-load 'gptel-transient
  ...
  (transient-define-suffix gptel--suffix-mcp-disconnect ()
    ...
    :transient t
    :inapt-if
    (lambda () (or (not (boundp 'mcp-hub--project-server-table))
              (null (gethash (project-current) mcp-hub--project-server-table))
              (cl-loop
               for v being the hash-values of (mcp-server-connections (project-current))
               never v)))
    (interactive)
